...
@section('title', config('app.name') . ' | Soal No.' . $no)
...
@section('content')
    <div class="container">
        <form
            action={{ request()->is('olympiad/penyisihan/*') ? route('answer_penyisihan', ['no' => $no]) : route('answer_semifinal', ['no' => $no]) }}
            method="POST" class="attempt">
            <div class="number-question-section">
                <section class="number-section">
                    ...
                    <label for="flag" class="flag" style="cursor: initial">
                        <center>
                            {{ $question->nilai > 1 ? $question->nilai . ' Points' : $question->nilai . ' Point' }}
                        </center>
                    </label>
                </section>
                        ...
                        @foreach ($options->shuffle() as $option)
                            <div class="form-radio">
                                ...
                                @if ($option->gambar_option)
                                    <img class="gambar"
                                        src="{{ asset('storage/gambar_option/' . $option->gambar_option) }}">
                                @endif
                            </div>
                        @endforeach
                        <div class="form-radio">
                            <input type="radio" class="hidden" name="option_id" id="reset" value="">
                            <label class="batal" for="reset">
                                Batalkan Pilihan
                            </label>
                        </div>
                    </div>
            ...
            <dialog class="modal-box">
                <div class="modal-box__wrapper">
                    {{-- <div class="gambar">
                        <img class="gambar-formal" src="{{ url('/assets/img/maskot-formal.png') }}" alt="">
                        <img class="gambar-formal" src="{{ url('/assets/img/maskot-formal.png') }}" alt="">
                        <img class="gambar-formal" src="{{ url('/assets/img/maskot-formal.png') }}" alt="">
                    </div> --}}
                    <div class="detail">
                        ...
                        <div class="tombol">
                            ...
                            <button type="submit" name="action" value="selesai" class="btn-primary">Yakin</button>
                        </div>
                    </div>
                </div>
            </dialog>
